// Modal de ingreso: Google.
//
// Fue quedando en una sola puerta a propósito. El código por correo prometía un
// mail que nadie mandaba, y el modo invitado dejaba entrar con un id inventado
// en el navegador: servía para mirar, pero no para usar el producto —las
// sugerencias y el banco de fotos piden cuenta— y del lado del servidor
// obligaba a aceptar un token que cualquiera se fabrica.
//
...
export function abrirModalAuth({ alAutenticar = () => {} } = {}) {
  ...
  function renderIngreso() {
    ...
    const btnGoogle = el('button.btn.btn-google', {
      ...
      onclick: async () => {
        ...
        try {
          ...
          msgError.textContent = e.message || 'No se pudo entrar con Google. Probá de nuevo en un momento.'
    ...
    caja.append(
      ...
      msgError,
      btnGoogle,
      el('p.apunte.chico', { style: 'margin-top:14px;text-align:center' },
        'Usamos tu cuenta de Google solo para reconocerte y guardar tu marca y tus placas.')
    )
