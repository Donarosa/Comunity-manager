/**
 * Fondo animado Mosaico 3D con GSAP 3 para la pantalla de edición (#editor-editar).
 * Despliega un muro de placas en perspectiva tridimensional (rotateX, rotateZ)
 * con movimiento continuo diagonal y micro-animaciones individuales.
 * Utiliza los tokens OKLCH del sistema de diseño Alquimia (Hum / Bubble Edition)
 * y combina placas institucionales con placas fotográficas.
 */
...
const PLACAS = [
  ...
  },
  {
    ...
    body: 'Las nuevas escalas, topes de facturación y fechas límite.',
    foot: '@ESTUDIORAVENNA',
  },
  {
    theme: 't-ink-deep',
  ...
  },
  {
    ...
    body: 'Frenos, fluidos y suspensión. Diagnóstico en 40 min.',
  ...
  },
  {
    ...
    body: 'Técnica balayage con productos sin amoníaco.',
    foot: '@BRUMAESTUDIO',
  },
  {
    theme: 't-pear',
    ...
    body: 'Directos del productor. Sin cámara de maduración.',
  ...
  },
  {
    ...
    body: 'Sin pesas, sin excusas. Rutina de 20 min en casa.',
    foot: '@FITNESSPRO.AR',
  },
  {
    theme: 't-cream',
...
function crearPlacaMosaico(data) {
...
/**
 * Monta el fondo animado Mosaico 3D dentro del elemento contenedor.
 * Retorna { destruir() } para limpiar las animaciones y el DOM.
 */
export function montarFondoMosaico(contenedor) {
  const gsap = window.gsap
  if (!gsap) {
    console.warn('[mosaico] GSAP no está disponible en window.gsap')
    return { destruir: () => {} }
  }

  const bgContenedor = document.createElement('div')
  bgContenedor.className = 'formato-mosaico-bg'

  const vignette = document.createElement('div')
  vignette.className = 'formato-mosaico-vignette'

  const perspectiveGrid = document.createElement('div')
  perspectiveGrid.className = 'mosaico-grid'

  bgContenedor.appendChild(perspectiveGrid)
  contenedor.prepend(vignette)
  contenedor.prepend(bgContenedor)

  const items = shuffle(PLACAS).concat(shuffle(PLACAS)).concat(shuffle(PLACAS))
  const count = 30

  for (let i = 0; i < count; i++) {
    const data = items[i % PLACAS.length]
    const card = crearPlacaMosaico(data)
    card.style.opacity = 0.35 + Math.random() * 0.25
    perspectiveGrid.appendChild(card)

    // Micro oscilación suave individual
    gsap.to(card, {
      y: -6 + Math.random() * 12,
  ...
  // Desplazamiento diagonal tridimensional continuo del muro
  const mainTween = gsap.to(perspectiveGrid, {
    x: -260,
    y: -200,
    duration: 38,
    ease: 'none',
  ...
  return {
    destruir() {
      mainTween?.kill()
      gsap.killTweensOf('.mosaico-grid')
      gsap.killTweensOf('.mosaico-grid .cascada-placa')
      bgContenedor.remove()
      vignette.remove()
    }
  }
